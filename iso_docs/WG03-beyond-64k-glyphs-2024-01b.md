...
The **TupleValues** format is essentially the same as @@link-to Packed
Deltas from the TupleVariationStore, but modified to allow storing
...
the CFF2Index format used in VARC (and hence in *MultiItemVariationData*
subtables) is a variable-sized CFF2-style Index structure, as defined in
\[@@CFFF2\]. CFF2Index is used to store a list of variable-sized data,
such as glyph records, in a space-efficient manner.
