...
The component glyphs to be loaded use coordinate values specified. For any unspecified axis, the value used depends on flag bit 14. If the flag is clear, then the currently set axis value from the font is used. If the flag is set the axis value from current glyph being processed (which itself might recursively come from the font or its own parent glyphs) are used.
